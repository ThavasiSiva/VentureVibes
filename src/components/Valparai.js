...
function Valparai() {
    ...
    const fetchItems = async () => {
      try {
        ...
        const filteredItems = response.data.filter(
          (item) =>
            item.category_name &&
            item.category_name.category_name === "Hill Station"
        );
  ...
  useEffect(() => {
    const getWeather = async (cities) => {
      try {
        const apiKey = "da3498ed976971af62685972558fcf2b"; // Replace with your OpenWeatherMap API key
    ...
    const cities = ["Valparai", "Pooluvapatti"]; // Add more cities as needed
    getWeather(cities);
  }, []);
  ...
  return (
    ...
    <div >
      <Container fluid>
        <h1  style={{ textAlign: "center", fontFamily:"serif",color:"rgb(136, 78, 78)"}}>
          Hill Station
        </h1>
        <hr  />
        <Row>
    ...
    <tr>
      ...
      <td>
        <div>
          {weatherData[index] && (
            <div>
              Temperature: {weatherData[index].data.main.temp}°C <br />
              Description: {weatherData[index].data.weather[0].description}
            </div>
          )}
        </div>
      </td>
